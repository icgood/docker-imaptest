...
RUN curl -LO ${dovecot_url} && tar xzf dovecot-latest.tar.gz && mv dovecot-0.0.0-* dovecot-latest

ARG imaptest_url="http://dovecot.org/nightly/imaptest/imaptest-latest.tar.gz"
RUN curl -LO ${imaptest_url} && tar xzf imaptest-latest.tar.gz && mv dovecot-*-imaptest-* imaptest-latest
